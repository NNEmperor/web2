...
@Component({
  selector: 'app-wr-tab-equipment',
  templateUrl: './wr-tab-equipment.component.html',
  styleUrls: ['./wr-tab-equipment.component.css']
})
export class WrTabEquipmentComponent implements OnInit {
  data: any[] = [];
  showingData: any[] = [];
  coords: any[] = [];
  ...
  constructor(public dialogRef: MatDialog, public dialog: MatDialog ,private service: MessagePassingService, private incidentService: IncidentPassingService, private servis:WorkReqServiceService ){
    this.service.changeData("WORK REQUEST - Update - Equipment")
  ...
  ngOnInit(): void {
    let pos;
     pos=localStorage.getItem("device-wr-edit")     //PAZNJA EDITOVO
     if(pos!=null){
       ...
       this.servis.GetDevices(idwr).subscribe(res=>{
          this.showingData=res as any[];//dodeli
  ...
  saveDevices(){
    ...
    console.log(json);
    console.log('----------')
    localStorage.setItem("device-wr-edit",json)
